...
class _TransactionContentState extends State<TransactionContent> {
  ...
  void initState() {
    super.initState();
    context.read<GetListTransactionBloc>().add(const GetListTransactionEvent.fetch());
  }
  ...
  Widget build(BuildContext context) {
    return BlocConsumer<GetListTransactionBloc, GetListTransactionState>(
      ...
      builder: (context, state) {
        ...
        if (state.status == GetListTransactionStatus.failure) return Center(child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [Text(state.error.toString()), const SizedBox(height: 8), ElevatedButton(onPressed: () => context.read<GetListTransactionBloc>().add(const GetListTransactionEvent.fetch()), child: const Text('Retry'))]));
        final all = state.data ?? [];
        if (all.isEmpty) return RefreshIndicator(onRefresh: () async => context.read<GetListTransactionBloc>().add(const GetListTransactionEvent.fetch()), child: ListView(children: const [SizedBox(height: 100), Center(child: Text('Belum ada transaksi')), Center(child: Icon(Icons.inbox, size: 64, color: Colors.grey))]));
